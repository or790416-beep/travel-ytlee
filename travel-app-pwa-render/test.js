...
api.state.trip.packing = [
  { id: 0, category: "衣物", items: [{ id: 0, name: "上衣", done: true }, { id: "keep", name: "外套", done: false }] },
  { id: "docs", category: "證件", items: [{ id: "passport", name: "護照", done: true }] },
];
let packingMarkup = api.renderPacking();
assert.match(packingMarkup, /data-menu-packing-manager[\s\S]*aria-label="管理打包清單項目欄"/);
assert.equal((packingMarkup.match(/data-menu-packing-category-id=/g) || []).length, 2);
assert.doesNotMatch(packingMarkup, /id="add-pack-item"|id="new-pack-item"|data-menu-pack-id/);
api.state.openMenuId = "packing-manager";
assert.match(api.renderMobileActionSheet(), /新增項目欄[\s\S]*刪除項目欄/);
api.state.openMenuId = "packing-category:0";
assert.match(api.renderMobileActionSheet(), /新增子項目[\s\S]*刪除子項目[\s\S]*重新命名項目欄/);
api.state.packingDialog = { type: "add-category" };
assert.match(api.renderPackingDialog(), /data-packing-dialog-form="add-category"[\s\S]*項目欄名稱/);
const writesBeforeCancel = storageWrites;
const packingBeforeCancel = JSON.stringify(api.state.trip.packing);
api.state.packingDialog = null;
assert.equal(storageWrites, writesBeforeCancel);
confirmResult = false;
assert.equal(api.deletePackingGroup("0"), false);
assert.equal(api.deletePackingChild("0", "0"), false);
...
const originalGroupIds = Array.from(api.state.trip.packing, (group) => group.id);
const addedGroup = api.addPackingGroup("電子用品");
assert.ok(addedGroup.id);
assert.notEqual(addedGroup.id, 0);
assert.deepEqual(Array.from(api.state.trip.packing.slice(0, 2), (group) => group.id), originalGroupIds);
const originalItems = JSON.stringify(api.state.trip.packing[0].items);
const addedItem = api.addPackingChild("0", "襪子");
assert.ok(addedItem.id);
assert.equal(addedItem.done, false);
assert.equal(JSON.stringify(api.state.trip.packing[0].items.slice(0, 2)), originalItems);
assert.equal(api.renamePackingGroup("docs", "重要證件"), true);
assert.equal(api.state.trip.packing[1].id, "docs");
assert.equal(api.state.trip.packing[1].items[0].id, "passport");
assert.equal(api.deletePackingChild("0", "0"), true);
assert.deepEqual(Array.from(api.state.trip.packing[0].items, (item) => [item.id, item.name, item.done]), [["keep", "外套", false], [addedItem.id, "襪子", false]]);
assert.equal(api.deletePackingGroup("docs"), true);
assert.deepEqual(Array.from(api.state.trip.packing, (group) => group.id), [0, addedGroup.id]);
assert.ok(storageWrites > writesBeforeCancel);
assert.equal(api.state.syncDirty, true);
assert.equal(api.findPackingGroup("missing"), null);
assert.equal(api.deletePackingGroup("missing"), false);
assert.equal(alertMessages.at(-1), "找不到這個打包項目欄，請重新開啟。");
assert.equal(api.deletePackingChild("0", "missing"), false);
assert.equal(alertMessages.at(-1), "找不到這筆打包項目，請重新開啟。");
assert.equal(api.deletePackingGroup("0"), true);
assert.equal(api.deletePackingGroup(addedGroup.id), true);
packingMarkup = api.renderPacking();
assert.match(packingMarkup, /管理打包清單項目欄/);
assert.match(packingMarkup, /尚無打包項目/);
api.state.syncSettings = null;
assert.match(styles, /\.reference-card-list\s*\{[\s\S]*?row-gap:\s*1px;[\s\S]*?margin:\s*0;[\s\S]*?padding:\s*0;/);
